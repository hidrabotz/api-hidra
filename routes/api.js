...
try {
var zahirr = db.get("hidra");
} catch (e) {
	console.log('')
}
var fs = require('fs');
var creatorList = ['@hidra'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'Não tem apikey? Chame e compre sua chave por apenas 10 R$ wa.me/5567996005372'
    },
    notkey: {
        status: false,
        criador: `${creator}`,
    ...
    invalidKey: {
        ...
        criador: `${creator}`,
        code: 406,
        mensagem: 'apikey invalida, Não tem apikey? Chame e compre sua chave por apenas 10 R$ wa.me/5567996005372'
    },
    invalidlink: {
        status: false,
        criador: `${creator}`,
...
const listkey = ["hidrabot", "supraz", "lhannaBot"];
var len = 15
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'hidra') return res.json(loghandler.invalidKey)
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){          
       fetch(encodeURI(`https://brasilapi.com.br/api/cnpj/v1/${cnpj}`))
        .then(response => response.json())
        .then(data => {
         var uf = data.uf;
        var cep = data.cep;
        var pais = data.pais;
       var nome_socio = data.nome_socio; 
       var codigo_pais = data.codigo_pais;
       var faixa_etaria = data.faixa_etaria;
       var cnpj_cpf_do_socio = data.cnpj_cpf_do_socio;
       var qualificacao_socio = data.qualificacao_socio;
       var codigo_faixa_etaria = data.codigo_faixa_etaria;
       var data_entrada_sociedade = data.data_entrada_sociedade;
       var identificador_de_socio = data.identificador_de_socio;
       var cpf_representante_legal = data.cpf_representante_legal;
       var nome_representante_legal = data.nome_representante_legal;
       var codigo_qualificacao_socio = data.codigo_qualificacao_socio;
       var qualificacao_representante_legal = data.qualificacao_representante_legal;
       var codigo_qualificacao_representante_legal = data.codigo_qualificacao_representante_legal;
       var cnpj = data.cnpj;
       var porte = data.porte;
       var numero = data.numero;
       var ddd_fax = data.ddd_fax;
       var municipio = data.municipio;
       var logradouro = data.logradouro;
       var cnae_fiscal = data.cnae_fiscal;
       var razao_social = data.razao_social;
       var nome_fantasia = data.nome_fantasia;
       var capital_social = data.capital_social;
       var ddd_telefone_1 = data.ddd_telefone_1;
       var ddd_telefone_2 = data.ddd_telefone_2;
       var descricao = data.descricao;
       var natureza_juridica = data.natureza_juridica;
       var situacao_cadastral = data.situacao_cadastral;
       var descricao_tipo_de_logradouro = data.descricao_tipo_de_logradouro;
       var descricao_motivo_situacao_cadastral = data.descricao_motivo_situacao_cadastral;
             res.json({
                 criador : `${creator}`,
                 uf,
                 cep,
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){    
    hasil = 'https://supra-api.herokuapp.com/api/ddd?quero=${quero}&apikey=supraz'
	  data = await fetch(hasil).then(v => v.buffer())   
...
router.get('/base', async (req, res, next) => {
	var type = req.query.type,
		...
		if (apikeyInput != 'hidra') return res.json(loghandler.invalidKey)
...
router.get('/validnum', async (req, res, next) => {
     num = req.query.num
    var Apikey = req.query.apikey           
	if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){
       fetch(encodeURI(`https://phonevalidation.abstractapi.com/v1/?api_key=af7a3c3217db443fa3b23ca34464f415&phone=${num}`))
        .then(response => response.json())
        .then(data => {
